...
# metaclass for automatically creating descriptors
class typedmeta(type):
    def __new__(cls,clsname,bases,methods):
        for key, descrip in methods.items():
            if isinstance(descrip,type) and issubclass(descrip, Typed):
                d = descrip(key)
                methods[key] = d
